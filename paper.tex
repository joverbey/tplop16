...
\begin{abstract}
Many automated refactoring tools provide an extensibility mechanism, which
...
soruce code.  Refactoring tools almost always use a data structure called an
...
refactoring tools impose more stringent requirements on ASTs---specifically,
the ASTs must be suitable for manipulating the user's source code.
...
semi-automated edits to source code.  You have provided a mechanism that allows
...
change \\
\centerline{\ttt{if (a < b) if (c < d) System.out.println("a");
...
source file: one child node describes the \ttt{package} statement, another
...
\includegraphics[scale=0.6]{jdt-ast.eps}
...
\subsection{Requirements of Refactoring ASTs}
\label{ss:reqs}
...
\begin{enumerate}
\item To be useful for refactoring, an AST must accurately and
precisely model the user's source code at a very fine level of detail.
...
Note that the manipulation code using a mutable AST is simpler, and it more
directly expresses the programmer's intention.  Manipulating source code using
offset/length information has a major disadvantage: it leads to ugly client
code.  The client code can easily become littered with variables holding
character offsets and methods that perform intricate string manipulations.
