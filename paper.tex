...
These trees are often used internally by
...
There are several basic requirements that a solution API must meet.
...
Any candidate solution would have to satisfy the API requirements while avoiding
the problems associated with mutable ASTs.
...
refactoring too must be able to parse, analyze, and transform code \textit{with embedded preprocessor directives.}
