...
\begin{abstract}
Many automated refactoring tools provide an extensibility mechanisms that
allows third parties to contribute new refactorings.  However, implementing
...
characteristics of good API design.  This pattern is used in the Eclipse Java
Development Tools' refactoring API as well as the Microsoft ``Roslyn'' CTP,
among other uses.
\end{abstract}
...
\terms{Languages}
\keywords{abstract syntax trees, application programming interfaces, APIs,
ASTs, immutability, refactoring}
...
%Preprocessors (such as the C preprocessor and M4) do not pose much of a problem
%for compilers.  In a compiler, the preprocessor can be run first, and then its
%output can serve as the input to the lexer/parser.  The preprocessor can be
%completely unaware of what lexer/parser it is feeding into, and the
%lexer/parser can be unaware of the fact that the code was preprocessed
%(although it might handle directives like \code{\#line} to give better error
%messages).
%
%Unfortunately, the C preprocessor makes refactoring very difficult; it is the
%subject of Garrido's entire Ph.D.~thesis~\cite{garrido05program}.  The
%compiler's approach of preprocesing the code first and then parsing it does not
%work.  Since refactoring tools must modify the \textit{user's} source code,
%they must be able to parse, analyze, and transform code \textit{with embedded
%preprocessor directives.}
%
%This dissertation will not treat the topic of refactoring preprocessed code in
%detail, but suffice it to say that a compiler's AST will almost certainly be
%inadequate.  At a minimum, a refactoring must be able to determine which AST
%nodes correspond to actual text in the user's source code and which ones are
%the result of file inclusions and macro expansions.  For more details, see
%Overbey et al.~\cite{wrt2009} and Garrido~\cite{garrido05program}.

%\subsection{Mapping AST Nodes to Source Locations}
%\label{ss:mapping}
...
suggest minimizing the amount of mutability in an API.  After all, immutable
...
\textit{if} statements, one nested inside the other.  Suppose the outer
\textit{if} statement were modified first: Adding braces to it would change the
position at which the closing brace for the inner \textit{if} statement would
need to be added.  Consider a somewhat simplified example.  Pictorially:

\begin{center}
\jb{i}\jb{f}\jb{(}\jb{1}\jb{)}\jb{ }%
\jb{i}\jb{f}\jb{(}\jb{2}\jb{)}\jb{ }%
\jb{s}\jb{(}\jb{)}\jb{;}\\
\lb{\,}\kb{0}\kb{1}\kb{2}\kb{3}\kb{4}\kb{5}\kb{6}\kb{7}\kb{8}\lb{9}%
\lb{10}\lb{11}\lb{12}\lb{13}\lb{14}\lb{15} \\
...
\noindent
The body of the inner \textit{if} statement begins at offset~12 in the original
code.  But after adding braces to the outer \textit{if} statement, the body of
the inner \textit{if} statement is shifted to offset~14.
...
Tools~\cite{jdt-astrewrite,cdt-refactoring} take this approach
(with slightly different class and method names).

\begin{figure}[!bt]
\begin{center}
\includegraphics[width=\textwidth]{seq-diagram.eps}
\end{center}
...
Eclipse~\cite{scalaide,scalarefactoring} takes a functional approach,
implementing refactorings as tree transformations.  Source generation is
handled separately: one printer uses position information to retain the
formatting of the original code, while another pretty-prints new nodes that
have been added to the AST~\cite{stocker10scala}.
...
\textit{The author would like to thank Peter Sommerlad for the invaluable
feedback and suggestions he provided as the shepherd for this paper.}
